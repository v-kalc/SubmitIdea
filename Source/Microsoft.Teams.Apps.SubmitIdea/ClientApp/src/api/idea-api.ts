...
export const getFilteredPosts = async (categories: string, sharedByNames: string, tags: string, sortBy: string, teamId: string, pageCount: number): Promise<any> => {
    let url = `${baseAxiosUrl}/idea/applied-filtered-team-ideas?categories=${encodeURIComponent(categories)}&sharedByNames=${sharedByNames}
...
export const getTags = async (): Promise<any> => {
    let url = `${baseAxiosUrl}/idea/unique-tags?searchText=*`;
    return await axios.get(url);
}
...
export const filterTitleAndTags = async (searchText: string, pageCount: number): Promise<any> => {
    let url = baseAxiosUrl + `/idea/search-ideas?pageCount=${pageCount}&searchText=${encodeURIComponent(searchText)}`;
    return await axios.get(url);
}
...
export const filterTitleAndTagsTeam = async (searchText: string, teamId: string, pageCount: number): Promise<any> => {
    let url = baseAxiosUrl + `/teamidea/team-search-ideas?teamId=${teamId}&pageCount=${pageCount}&searchText=${encodeURIComponent(searchText)}`;
    return await axios.get(url);
}
